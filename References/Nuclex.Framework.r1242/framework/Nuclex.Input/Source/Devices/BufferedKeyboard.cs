...
namespace Nuclex.Input.Devices {
  ...
  public abstract class BufferedKeyboard : IKeyboard {
    ...
    internal BufferedKeyboard() {
      ...
      this.current = new ModifiableKeyboardState();
    }
      ...
      while (this.queuedEvents.Count > 0) {
        ...
        switch (nextEvent.EventType) {
          case EventType.KeyRelease: {
            this.current.AddPressedKey((int)nextEvent.Key);
            OnKeyReleased(nextEvent.Key);
            break;
          }
          case EventType.KeyPress: {
            this.current.RemovePressedKey((int)nextEvent.Key);
    ...
    private ModifiableKeyboardState current;
